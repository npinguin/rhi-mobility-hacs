...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    data=hass.data[DOMAIN][entry.entry_id]; controller=data['controller']; manager=data['runtime']; registry=data['registry']; created={}
    @callback
    def sync() -> None:
        new=[]; wanted=set()
        for asset_id,asset in sorted(manager.assets.items()):
            for property_key,editable in editable_definitions(registry,asset.concept_id,'number'):
                ...
                if key not in created:
                    entity=MobilityNumber(entry.entry_id,asset_id,property_key,editable,manager,controller,registry); created[key]=entity; new.append(entity)
...
class MobilityNumber(NumberEntity):
    ...
    def __init__(self,entry_id,asset_id,property_key,editable,manager,controller,registry):
        ...
        suffix=property_key.replace('.','_'); self._attr_unique_id=f'{DOMAIN}:{asset_id}:number:{property_key}'; self._attr_suggested_object_id=f'{DOMAIN}_{asset_id}_{suffix}'; self._attr_name=editable.get('name') or property_key.split('.')[-1].replace('_',' ').title()
        definition=registry.semantic_catalog['properties'].get(property_key,{}); self._attr_native_unit_of_measurement=definition.get('unit'); self._attr_device_info=logical_device_info(manager.hass,entry_id,manager,asset_id)
    async def async_added_to_hass(self):
        self.async_on_remove(self.manager.add_asset_listener(self.asset_id,self._changed))
        if self.editable.get('write_kind') in {'charger_requested_power','vehicle_requested_power','charger_requested_current'}: self.async_on_remove(self.controller.add_listener(self._changed))
    @callback
    ...
    def extra_state_attributes(self):
        ...
        if kind in {'charger_requested_power','vehicle_requested_power'}:
            target=self.asset_id if kind=='charger_requested_power' else self.manager.effective_charger_for_vehicle(self.asset_id); status={} if not target else self.controller.requested_power_status(target)
    ...
    async def async_set_native_value(self,new_value: float): await async_write(self.manager,self.controller,self.asset_id,self.property_key,self.editable,float(new_value))

...
class MobilityLegacyV1ProjectedFacadeProvider(MobilityLegacyV1FacadeProvider):
    """Frozen V1 shape projected from typed V2 truth, never a second product model."""
    ...
    def property_rows(self,asset_type: str,component: str | None=None) -> list[dict[str,Any]]:
        ...
        for aid,asset in sorted(self.manager.assets.items()):
            ...
            for definition in self.defs_by_type.get(asset_type,[]):
                ...
                projection_key=definition.get("projection_property_key") or self._projection_key(aid,key)
                ...
                row=dict(projected)
                value=row.get("value")
                write=self._write_metadata(aid,key)
                row.update({
                    "property_key":key,
                    "value":value,
                    "display_value":value,
                    "friendly_name":definition.get("friendly_name") or row.get("friendly_name") or key,
                    "display_name":definition.get("friendly_name") or row.get("display_name") or key,

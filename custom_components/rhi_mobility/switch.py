...
class MobilityCompatibilitySwitch(SwitchEntity):
    ...
    def __init__(self,entry_id,asset_id,property_key,editable,manager,controller):
        ...
        suffix=property_key.replace('.','_'); self._attr_unique_id=f'{DOMAIN}:{asset_id}:switch:{property_key}'; self._attr_suggested_object_id=f'{DOMAIN}_{asset_id}_{suffix}'; self._attr_name=editable.get('name') or property_key.split('.')[-1].replace('_',' ').title(); self._attr_device_info=logical_device_info(manager.hass,entry_id,manager,asset_id)
    async def async_added_to_hass(self): self.async_on_remove(self.manager.add_asset_listener(self.asset_id,self._changed))
    @callback
    ...
    def extra_state_attributes(self):
        if self.editable.get('write_kind')=='lifecycle_enabled_alias': return {'compatibility_property':'asset.enabled','canonical_owner':'asset.lifecycle_status','second_lifecycle_owner':False}
